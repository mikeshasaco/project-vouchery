...
@section('content')
    ...
    @if(count($products) == "0")
    <div class="col-md-12">
        <h3 class="nocategoryproducts" align="center" >No Products found under <b style="color:red">{{$catbyuser}}</b> Category.</h3>
    </div>
    @else
...
@section('javascripts')
    ...
    $("[name='record']").on("change", function (e) {
        let slug = $(this).val();
        window.location.href = window.location.origin + '/businesses/category'+'/'+ slug;
    });
    </script>
    <script>
